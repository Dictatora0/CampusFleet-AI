...
class DataVisualizer:
    ...
    def load_data(self, frames_csv: str, cars_csv: str, 
                  orders_csv: str = None) -> tuple:
        """
        加载CSV数据
        Args:
            frames_csv: 帧数据CSV路径
    ...
    def plot_order_completion_over_time(self, frames_df: pd.DataFrame, 
        ...
        ax.plot(frames_df['step'], frames_df['completed_orders'], 
                label='Completed Orders', linewidth=2, color='green')
        ax.plot(frames_df['step'], frames_df['pending_orders'], 
                label='Pending Orders', linewidth=2, color='orange')
        ax.plot(frames_df['step'], frames_df['assigned_orders'], 
                label='Assigned Orders', linewidth=2, color='blue')
    ...
    def plot_efficiency_metrics(self, frames_df: pd.DataFrame, 
                                save_path: Optional[str] = None):
        """
        ...
        # 订单完成效率
        ax1.plot(frames_df['step'], frames_df['completion_rate'], 
        ...
        # 车辆利用率
        ax2.plot(frames_df['step'], frames_df['utilization_rate'], 
                linewidth=2, color='blue')
        ax2.fill_between(frames_df['step'], 0, frames_df['utilization_rate'], 
    ...
    def plot_distance_statistics(self, cars_df: pd.DataFrame, 
                                 save_path: Optional[str] = None):
        """
        ...
        ax1.bar(final_data['car_id'].astype(str), final_data['total_distance'], 
        ...
        for i, (idx, row) in enumerate(final_data.iterrows()):
            ax1.text(i, row['total_distance'] + 0.5, str(int(row['total_distance'])), 
                    ha='center', va='bottom', fontsize=10, fontweight='bold')
        
        # 距离随时间变化
        for car_id in cars_df['car_id'].unique():
            car_data = cars_df[cars_df['car_id'] == car_id]
            ax2.plot(car_data['step'], car_data['total_distance'], 
    ...
    def plot_heatmap(self, cars_df: pd.DataFrame, grid_size: int = 15,
                    save_path: Optional[str] = None):
        """
        绘制车辆访问热力图
        Args:
    ...
    def plot_order_waiting_time(self, orders_df: pd.DataFrame,
                                save_path: Optional[str] = None):
        """
        绘制订单等待时间分布图
        Args:
        ...
        ax1.axvline(avg_wait, color='red', linestyle='--', linewidth=2, label=f'Mean: {avg_wait:.1f}')
        ax1.axvline(median_wait, color='green', linestyle='--', linewidth=2, label=f'Median: {median_wait:.1f}')
        ax1.legend()
        
        # 箱型图
        ax2.boxplot(waiting_times, vert=True, patch_artist=True,
        ...
        ax2.text(1.15, max_wait * 0.5, stats_text, fontsize=11, 
    ...
    def generate_all_plots(self, frames_csv: str, cars_csv: str, 
                          orders_csv: str = None, grid_size: int = 15):
        """
        生成所有图表
        Args:
            frames_csv: 帧数据CSV路径
