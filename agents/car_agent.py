...
class CarAgent:
    ...
    def plan_path(
        self, target: Tuple[int, int], pathfinder, blocked_positions: Optional[Set] = None
    ):
        """
        规划到目标点的路径
        Args:
    ...
    def step(self, pathfinder, other_car_positions: Set[Tuple[int, int]]) -> bool:
        ...
        if self.state == CarState.WAITING:
            ...
            if self.wait_counter >= self.max_wait_time:
                ...
                self.wait_counter = 0
                self.state = (
                    CarState.MOVING_TO_PICKUP if self.pickup_point else CarState.MOVING_TO_DELIVERY
                )
                self.current_path = []
            return False
    ...
    def handle_collision(
        self, other_car_id: int, pathfinder, other_car_positions: Set[Tuple[int, int]]
    ) -> bool:
        """
        处理碰撞/冲突：按ID优先级决定
        Args:
    ...
    def _smart_avoidance(
        self,
        target: Tuple[int, int],
        blocked_position: Tuple[int, int],
        pathfinder,
        other_car_positions: Set[Tuple[int, int]],
    ) -> bool:
        """
        智能避让策略：尝试多种避让方案
        Args:
    ...
    def _handle_deadlock(self, pathfinder, other_car_positions: Set[Tuple[int, int]]) -> bool:
        ...
        # 策略 3：完全重置路径规划
        target = (
            self.pickup_point if self.state == CarState.MOVING_TO_PICKUP else self.delivery_point
        )
        if target:
            self.current_path = []
            self.replan_attempts = 0
