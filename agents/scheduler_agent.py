...
class SchedulerAgent:
    ...
    def __init__(
        self,
        strategy: SchedulingStrategy = SchedulingStrategy.GREEDY_NEAREST,
        grid_size: int = 15,
        max_cars: int = 10,
        max_orders: int = 20,
    ):
    ...
    def _greedy_nearest_schedule(self, cars: List, orders: List, grid_env) -> List[Tuple]:
        ...
        for order in orders:
            ...
            if best_car:
                assignments.append(
                    (best_car.car_id, order.order_id, order.pickup_point, order.delivery_point)
                )
        ...
        if assignments:
            self.assignment_history.append(
                {"assignments": assignments, "strategy": self.strategy.value}
            )
    ...
    def _hungarian_schedule(self, cars: List, orders: List, grid_env) -> List[Tuple]:
        ...
        for car_idx, order_idx in zip(row_indices, col_indices):
            ...
            assignments.append(
                (car.car_id, order.order_id, order.pickup_point, order.delivery_point)
            )
            self.total_assignments += 1
        ...
        for vehicle_id, route in routes.items():
            ...
            for order_id, locations in order_groups.items():
                if locations["pickup"] and locations["delivery"]:
                    assignments.append(
                        (vehicle_id, order_id, locations["pickup"], locations["delivery"])
                    )
                    self.total_assignments += 1
            ...
            if coordinated_paths:
                ...
                for car_id, path in coordinated_paths.items():
                    if car_id in car_order_mapping:
                        ...
                        assignments.append(
                            (car_id, order.order_id, order.pickup_point, order.delivery_point)
                        )
                        self.total_assignments += 1
                ...
                # 为当前批次使用贪心分配
                greedy_assignments = self._greedy_nearest_schedule(
                    current_cars, current_orders, grid_env
                )
