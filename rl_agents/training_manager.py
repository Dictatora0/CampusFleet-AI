...
class EvaluationMetrics:
    ...
    def add_episode(self, reward: float, length: int, completion_rate: float,
                   avg_distance: float, efficiency_score: float):
        """添加episode结果"""
...
class TrainingManager:
    ...
    def _run_episode(self, training: bool = True) -> Dict[str, Any]:
        ...
        while True:
            ...
            if self.agent_type.upper() == "DQN":
                ...
                # 转换为MultiDiscrete动作
                action = np.array([action_int % self.env.action_space.nvec[i] 
    ...
    def _check_early_stop(self) -> bool:
        ...
        # 如果成功率达到阈值且没有改进
        if (recent_stats['success_rate'] >= self.train_config['early_stop_threshold'] and
            self.no_improvement_count >= self.train_config['patience']):
            return True
    ...
    def _save_checkpoint(self, episode: int):
        """保存训练检查点"""
        checkpoint_path = os.path.join(self.save_dir, f"{self.agent_type.lower()}_checkpoint_ep{episode}.pth")
    ...
    def _save_training_results(self, final_stats: Dict, training_time: float):
        """保存训练结果"""
        # 保存指标CSV
        metrics_path = os.path.join(self.save_dir, f"{self.agent_type.lower()}_training_metrics.csv")
        ...
        results_path = os.path.join(self.save_dir, f"{self.agent_type.lower()}_training_results.json")
    ...
    def plot_training_progress(self, save_path: Optional[str] = None):
        ...
        axes[0, 0].plot(episodes, pd.Series(self.metrics.episode_rewards).rolling(50).mean(), 'r-', linewidth=2)
        ...
        axes[0, 1].plot(episodes, pd.Series(self.metrics.completion_rates).rolling(50).mean(), 'g-', linewidth=2)
        ...
        axes[1, 0].plot(episodes, pd.Series(self.metrics.avg_distances).rolling(50).mean(), 'b-', linewidth=2)
        ...
        axes[1, 1].plot(episodes, pd.Series(self.metrics.success_rates).rolling(50).mean(), 'orange', linewidth=2)
        ...
        if save_path:
            ...
            plt.savefig(os.path.join(self.save_dir, f"{self.agent_type.lower()}_training_progress.png"), 
    ...
    def compare_with_baseline(self, baseline_stats: Dict[str, float]) -> Dict[str, float]:
        ...
        for key in baseline_stats:
            if key in recent_stats:
                improvement = (recent_stats[key] - baseline_stats[key]) / max(baseline_stats[key], 1e-6)
