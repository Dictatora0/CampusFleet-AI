...
class StateEncoder:
    ...
    def encode_state(self, context: SimulationContext) -> np.ndarray:
        ...
        for i, car in enumerate(cars):
            if i < self.max_cars:
                # 位置信息 (归一化到 [0, 1])
                state[idx:idx+2] = [car.position[0] / self.grid_size, 
...
class RewardCalculator:
    ...
    def calculate_reward(self, context: SimulationContext, 
                        action_taken: Dict, 
                        prev_state: Optional[Dict] = None) -> float:
        """
        计算即时奖励
        ...
        # 1. 订单完成奖励
        if self.prev_stats.get('total_completed_orders', 0) < current_stats.get('total_completed_orders', 0):
            completed_diff = current_stats.get('total_completed_orders', 0) - self.prev_stats.get('total_completed_orders', 0)
    ...
    def get_episode_reward_summary(self, context: SimulationContext) -> Dict[str, float]:
        ...
        return {
            'total_completed': current_stats.get('total_completed_orders', 0) - start_stats.get('total_completed_orders', 0),
            'efficiency_change': (start_stats.get('avg_distance_per_order', 0) - current_stats.get('avg_distance_per_order', 0)),
            'total_distance': current_stats.get('total_distance', 0) - start_stats.get('total_distance', 0),
...
class RLEnvironment(gym.Env):
    ...
    def _check_terminated(self) -> bool:
        ...
        return pending_orders == 0 and all_cars_idle and self.total_orders_generated >= self.max_orders_per_episode
