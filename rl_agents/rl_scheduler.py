...
from .rl_environment import StateEncoder, RewardCalculator
...
class RLScheduler:
    ...
    def _select_rl_actions(self, state: np.ndarray, idle_cars: List, 
                          orders: List[Order]) -> List[Tuple]:
        """
        使用RL智能体选择动作并转换为调度分配
        ...
        try:
            ...
            if self.agent_type.upper() == "DQN":
                ...
                if hasattr(self, 'prev_action_int') and self.prev_log_prob and self.prev_value:
                    self.agent.store_experience(
                        self.prev_state, self.prev_action_int, 
                        self.prev_log_prob, reward, self.prev_value, done
                    )
                    ...
                    if len(self.agent.memory.states) >= 32 or done:
                        ...
                        if stats and self.agent.episodes % 10 == 0:
                            print(f"   PPO训练: Episodes={self.agent.episodes}, "
    ...
    def _fallback_schedule(self, idle_cars: List, orders: List[Order]) -> List[Tuple]:
        ...
        for car in idle_cars[:3]:  # 限制数量
            ...
            for order in orders:
                distance = abs(car.position[0] - order.pickup_point[0]) + abs(car.position[1] - order.pickup_point[1])
