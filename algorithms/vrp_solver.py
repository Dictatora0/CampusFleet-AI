...
class VRPSolver:
    ...
    def _greedy_assignment(
        self, vehicles: List, tasks: List[PickupDeliveryTask], distance_func
    ) -> Dict[int, VehicleRoute]:
        """
        贪心任务分配算法
        """
        routes = {}
        ...
        for vehicle in vehicles:
            ...
            while (
                available_tasks
                and len([t for t in route.tasks if t.task_type == TaskType.PICKUP])
                < self.vehicle_capacity
            ):
                best_task = self._select_best_task(
                    vehicle.position, route, available_tasks, distance_func
                )
    ...
    def _select_best_task(
        ...
        route: VehicleRoute,
        available_tasks: List[PickupDeliveryTask],
        distance_func,
    ) -> Optional[PickupDeliveryTask]:
        """
        选择最佳下一个任务
        优先考虑取货任务，选择距离最近的
        """
    ...
    def _find_delivery_task(
        self, order_id: str, available_tasks: List[PickupDeliveryTask]
    ) -> Optional[PickupDeliveryTask]:
        """查找对应的送货任务"""
    ...
    def _insert_delivery_task(
        self, route: VehicleRoute, delivery_task: PickupDeliveryTask, distance_func
    ):
        """
        将送货任务插入到路线的最佳位置
        必须在对应取货任务之后
        ...
        for pos in range(pickup_index + 1, len(route.tasks) + 1):
            ...
            distance_increase = self._calculate_insertion_cost(
                route, delivery_task, pos, distance_func
            )
    ...
    def _calculate_insertion_cost(
        self, route: VehicleRoute, task: PickupDeliveryTask, position: int, distance_func
    ) -> float:
        """计算在指定位置插入任务的代价"""
        if position == 0:
            if not route.tasks:
        ...
        new_distance = distance_func(prev_task.location, task.location) + distance_func(
            task.location, next_task.location
        )
                ...
                for j in range(i + 2, len(optimized_route.tasks)):
                    ...
                    if self._is_valid_swap(optimized_route.tasks, i, j):
                        ...
                        test_route = VehicleRoute(
                            vehicle_id=optimized_route.vehicle_id, tasks=new_tasks
                        )
