...
class Conflict:
    ...
    def __hash__(self):
        return hash(
            (self.type, self.time, self.agent1, self.agent2, self.location1, self.location2)
        )
...
class SpaceTimeAStar:
    ...
    def plan_path(
        self, start: Tuple[int, int], goal: Tuple[int, int], constraints: Set[Constraint]
        ...
        for constraint in constraints:
            if constraint.type == ConflictType.VERTEX:
                ...
                edge_constraints.add(
                    (constraint.time, constraint.forbidden_from, constraint.location)
                )
        ...
        while open_list:
            ...
            successors = self._get_successors(
                current_pos, current_time, vertex_constraints, edge_constraints
            )
    ...
    def _get_successors(
        self, pos: Tuple[int, int], time: int, vertex_constraints: Set, edge_constraints: Set
    ) -> List[Tuple[int, int]]:
        """获取有效的后继位置"""
        successors = []
...
class ConflictBasedSearch:
    ...
    def plan_paths(
        self, agents: List, goals: Dict[int, Tuple[int, int]]
    ...
    def _create_child_nodes(self, parent: CBSNode, conflict: Conflict) -> List[Optional[CBSNode]]:
        ...
        locations = [
            conflict.location1,
            conflict.location2 if conflict.location2 else conflict.location1,
        ]
        ...
        for i, (agent, location) in enumerate(zip(agents, locations)):
            ...
            if conflict.type == ConflictType.VERTEX:
                constraint = Constraint(
                    agent=agent, time=conflict.time, location=location, type=ConflictType.VERTEX
                )
            else:  # EDGE
                constraint = Constraint(
                    agent=agent,
