    ...
    try:
        while viewer.running and step < max_steps:
            ...
            if step % 15 == 0:
                ...
                print(f"📊 步骤 {step:3d}: 完成 {completed:2d} 单, "
                      f"CBS调用 {cbs_calls} 次")
...
def demo_algorithm_comparison():
    ...
    for strategy, name in algorithms:
        ...
        while steps < max_steps:
            ...
            # 检测死锁（车辆长时间不动）
            stuck_cars = sum(1 for car in context.cars 
                           if hasattr(car, 'stuck_counter') and car.stuck_counter > 5)
            if stuck_cars > 0:
                deadlocks += 1
        ...
        print(f"  ✅ {steps}步完成 {stats['total_completed_orders']}单，"
    ...
    for result in results:
        print(f"{result['algorithm']:<20} {result['steps']:<6} {result['completed']:<6} "
