...
class TestPathFindingBasics:
    ...
    def test_is_valid_position(self, pathfinder_simple):
        """测试位置有效性检查"""
        # 有效位置
        assert pathfinder_simple.is_valid((0, 0)) == True
        assert pathfinder_simple.is_valid((4, 4)) == True
        
        # 无效位置（越界）
        assert pathfinder_simple.is_valid((-1, 0)) == False
        assert pathfinder_simple.is_valid((5, 0)) == False
        assert pathfinder_simple.is_valid((0, -1)) == False
        assert pathfinder_simple.is_valid((0, 5)) == False
        
        # 无效位置（障碍物）
        assert pathfinder_simple.is_valid((1, 1)) == False  # '#'
        assert pathfinder_simple.is_valid((1, 2)) == False  # '#'
