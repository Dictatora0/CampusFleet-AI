...
from agents import SchedulingStrategy, CarAgent
...
def test_pygame_visualization():
    ...
    try:
        ...
        print("✅ Pygame模块导入成功")
        print("⚠️  跳过实际渲染测试（需要图形界面）")
        print("✅ Pygame可视化模块可用！")
        
    except ImportError:
        print("⚠️  Pygame未安装，跳过可视化测试")
...
def test_scheduler_with_battery():
    ...
    if pending_orders:
        assignments = context.scheduler.schedule(
            context.cars, pending_orders, context.grid_env
        )
