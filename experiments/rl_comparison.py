...
def run_comparison_experiment():
    ...
    trainer_dqn = TrainingManager(
        ...
        agent_config=dqn_config,
        output_dir='experiments/results/standard_dqn'
    )
    ...
    trainer_double_dqn = TrainingManager(
        ...
        environment_config=env_config,
        training_config=training_config,
        agent_config=double_dqn_config,
        output_dir='experiments/results/double_dqn'
    )
...
def plot_comparison(results_dqn, results_double_dqn):
    ...
    if 'losses' in results_dqn:
        # 平滑处理
        window = 10
        dqn_losses_smooth = np.convolve(results_dqn['losses'], np.ones(window)/window, mode='valid')
        ax.plot(dqn_losses_smooth, label='标准 DQN', alpha=0.7, linewidth=2)
    if 'losses' in results_double_dqn:
        double_dqn_losses_smooth = np.convolve(results_double_dqn['losses'], np.ones(window)/window, mode='valid')
...
def save_comparison_results(results_dqn, results_double_dqn, env_config, training_config):
    ...
    def get_stats(results, name):
        ...
        return {
            ...
            'best_reward': float(max(results['episode_rewards'])) if results['episode_rewards'] else 0,
    ...
    if dqn_stats.get('final_avg_reward', 0) > 0:
        comparison_data['improvement']['reward'] = (
            ...
            dqn_stats.get('final_avg_reward', 1) * 100
        )
    ...
    if dqn_stats.get('final_avg_distance', 0) > 0:
        comparison_data['improvement']['distance'] = (
            ...
            dqn_stats.get('final_avg_distance', 1) * 100
        )
