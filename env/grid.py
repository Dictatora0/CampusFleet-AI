...
class GridEnvironment:
    ...
    def __init__(self, size: int = 15, num_charging_stations: int = 2):
        ...
        # 充电站管理器
        self.charging_station_manager = ChargingStationManager()
        self._initialize_charging_stations(num_charging_stations)

        # 兼容旧接口：保留位置列表
        self.charging_stations: List[Tuple[int, int]] = [
            station.position for station in self.charging_station_manager.stations.values()
        ]
    ...
    def _initialize_charging_stations(self, num_stations: int):
        ...
        if edge_positions:
            ...
            # 创建ChargingStation对象并添加到管理器
            for idx, position in enumerate(selected):
                station = ChargingStation(
                    station_id=idx, position=position, capacity=2, charging_rate=5.0  # 每个站2个充电位
                )
                self.charging_station_manager.add_station(station)
    ...
    def get_charging_station_manager(self) -> ChargingStationManager:
        """获取充电站管理器"""
        return self.charging_station_manager

    def get_nearest_charging_station(self, pos: Tuple[int, int]) -> Optional[Tuple[int, int]]:
        """
        获取离指定位置最近且可用的充电站（考虑排队）
        Args:
            pos: 当前位置
        Returns:
            最优充电站位置
        """
        station = self.charging_station_manager.get_nearest_available_station(pos)
        return station.position if station else None
