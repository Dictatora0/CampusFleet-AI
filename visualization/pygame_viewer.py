...
class PygameViewer:
    ...
    def draw_grid(self, grid: list, charging_stations: list):
        ...
        for pos in charging_stations:
            ...
            for radius_offset in range(3, 0, -1):
                ...
                pygame.draw.circle(s, (*self.COLORS['charging_glow'], alpha), 
                                 (self.cell_size // 2, self.cell_size // 2), radius)
                self.screen.blit(s, (j * self.cell_size, offset_y + i * self.cell_size))
            
            # 绘制主体
            pygame.draw.circle(self.screen, self.COLORS['charging_station'],
                             (x, y), self.cell_size // 3)
            
            # 绘制边框
            pygame.draw.circle(self.screen, (200, 150, 0),
                             (x, y), self.cell_size // 3, 2)
            
            # 绘制闪电图标
            bolt_points = [
                (x - 4, y - 8), (x + 2, y - 2), (x - 2, y + 2), (x + 4, y + 8),
                (x, y + 2), (x + 2, y - 4)
            ]
            # pygame.draw.polygon(self.screen, (255, 255, 255), bolt_points)
    
    def draw_car(self, car_pos: Tuple[int, int], car_id: int, state: str, 
        ...
        pygame.draw.rect(self.screen, battery_color,
                        (battery_x, battery_y, fill_width, battery_bar_height))
    ...
    def draw_order_markers(self, orders: list):
        ...
        for order in orders:
            ...
            pygame.draw.circle(self.screen, self.COLORS['order_pickup'],
                             (pickup_x, pickup_y), 5)
            ...
            pygame.draw.circle(self.screen, self.COLORS['order_delivery'],
                             (delivery_x, delivery_y), 5)
    ...
    def draw_info_panel(self, context):
        ...
        pygame.draw.rect(self.screen, self.COLORS['panel_bg'],
                        (panel_x, panel_y, panel_width, self.grid_height))
        
        # 绘制垂直分隔线
        pygame.draw.line(self.screen, self.COLORS['grid_line'],
                        (panel_x, panel_y), (panel_x, panel_y + self.grid_height), 2)
        ...
        # 标题卡片
        card_y = y_offset
        pygame.draw.rect(self.screen, (240, 242, 245),
                        (panel_x + 10, card_y, panel_width - 20, 40), border_radius=8)
        ...
        stats = [
            ...
            ("Pending", str(context.order_agent.report()['pending_orders']), self.COLORS['warning']),
        ]
        ...
        # 车辆状态卡片
        pygame.draw.rect(self.screen, (240, 242, 245),
                        (panel_x + 10, y_offset, panel_width - 20, 30), border_radius=8)
        ...
        for car in context.cars:
            # 车辆卡片背景
            card_height = 50
            pygame.draw.rect(self.screen, (248, 249, 250),
            ...
            pygame.draw.rect(self.screen, (220, 220, 220),
                           (bar_x, bar_y, bar_width, bar_height), border_radius=9)
            ...
            if fill_width > 0:
                pygame.draw.rect(self.screen, bar_color,
                               (bar_x, bar_y, fill_width, bar_height), border_radius=9)
    ...
    def draw_top_bar(self, context):
        ...
        pygame.draw.line(self.screen, (80, 80, 80), 
                        (0, header_height - 1), (self.window_width, header_height - 1), 2)
